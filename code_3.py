...
Starter baseline script for the Short-Horizon Return Prediction challenge.

What this placeholder does:
1. Loads train/test parquet files.
2. Trains a cross-validated baseline regressor.
3. Reports OOF R2.
4. Writes a valid submission CSV with ID,TARGET.

You can replace pieces marked TODO with stronger feature engineering or models.
"""
...
def load_competition_data(data_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    """Load train/test files and return feature columns."""
    train_path = data_dir / "train.parquet"
    test_path = data_dir / "test.parquet"

    if not train_path.exists() or not test_path.exists():
        raise FileNotFoundError("Expected train.parquet and test.parquet in data directory")
...
def train_cv_baseline(
    ...
    """Train fold models and return models plus out-of-fold R2."""
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(x), start=1):
        ...
        oof[va_idx] = val_pred
        fold_r2 = r2_score(y[va_idx], val_pred)
        print(f"Fold {fold}/{n_splits} R2: {fold_r2:.6f}")
        models.append(model)

    overall_r2 = r2_score(y, oof)
    return models, float(overall_r2)


def predict_mean_ensemble(models: list[HistGradientBoostingRegressor], x: np.ndarray) -> np.ndarray:
...
def run_pipeline(
    ...
    train_df, test_df, feature_cols = load_competition_data(data_dir)

    if max_train_rows is not None and 0 < max_train_rows < len(train_df):
        train_df = train_df.sample(n=max_train_rows, random_state=random_seed).reset_index(drop=True)
        print(f"Using sampled train rows: {len(train_df):,}")
    ...
    # TODO: Add feature engineering before training if needed.
    models, oof_r2 = train_cv_baseline(x_train, y_train, n_splits=folds, random_seed=random_seed)
    print(f"Overall OOF R2: {oof_r2:.6f}")

    # TODO: Replace this baseline ensemble with stronger stacked/blended models.
    test_preds = predict_mean_ensemble(models, x_test)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baseline placeholder for short-horizon return prediction")
    parser.add_argument("--data-dir", type=Path, default=Path("."), help="Directory containing train.parquet/test.parquet")
    parser.add_argument("--output", type=Path, default=Path("submission_code3.csv"), help="Output submission CSV path")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional row cap for faster local experiments",
    )
...
# Custom LightGBM callback: prints a line every `period` trees with elapsed time
def lgb_verbose(period=100, label=""):
    """Prints tree index, val-MAE and elapsed time every `period` rounds."""
    start = [time.time()]
    def _cb(env):
        it = env.iteration + 1
        if it == 1 or it % period == 0:
            elapsed = time.time() - start[0]
            # env.evaluation_result_list = [(dataset_name, metric, value, is_higher_better), ...]
            if env.evaluation_result_list:
                metric_str = "  ".join(
                    f"{r[0]}-{r[1]}={r[2]:.6f}" for r in env.evaluation_result_list
                )
            else:
                metric_str = ""
            p(f"      {label}tree {it:>4}  {metric_str}  elapsed={elapsed:.0f}s")
    _cb.order = 10
    return _cb
...
warmup_params = dict(
    ...
    n_estimators      = 2000,      # hard cap — enough to find plateau
    learning_rate     = 0.08,      # fast lr so we see convergence quickly
    num_leaves        = 127,
...
p("    Warm-up training (lr=0.08, max 2000 trees, early-stop 40) …")
p("    Printing every 50 trees — you will see output within ~30s")
...
wu_model.fit(
    ...
    callbacks=[
        ...
        lgb_verbose(period=50, label="[warmup] "),
    ],
)
BEST_N_EST = max(wu_model.best_iteration_, 100)
wu_r2 = r2_score(y_raw[va0], target_inverse(wu_model.predict(X_train[va0])))
p(f"    Warm-up done in {time.time()-t_wu:.1f}s")
p(f"    Best n_estimators @ lr=0.08 : {BEST_N_EST}")
p(f"    Warm-up fold R²             : {wu_r2:.6f}")

# Scale trees to lr=0.05 used in full CV  (0.08/0.05 = 1.6x)
# Hard cap at 2000 — beyond this, gains are marginal on this dataset
N_EST_FINAL = min(int(BEST_N_EST * 1.6) + 50, 2000)
p(f"    n_estimators for full CV (lr=0.05): {N_EST_FINAL}")
...
LGB_PARAMS = dict(
    ...
    learning_rate     = 0.05,      # faster than 0.02 — N_EST already scaled
    num_leaves        = 127,
    min_child_samples = 50,
    subsample         = 0.8,
    subsample_freq    = 1,
...
def train_lgb(params, X, y_t, X_te, fold_list, label="LGB"):
    ...
    for fold, (tr_idx, va_idx) in enumerate(fold_list):
        ...
        p(f"    [{label}] Fold {fold+1}/{len(fold_list)} — training started …")
        model.fit(
            X[tr_idx], y_t[tr_idx],
            eval_set=[(X[va_idx], y_t[va_idx])],
            callbacks=[
                lgb.early_stopping(stopping_rounds=80, verbose=False),
                lgb_verbose(period=100, label=f"[{label} f{fold+1}] "),
            ],
        )
        ...
        p(f"    [{label}] Fold {fold+1}/{len(fold_list)}  "
          ...
          f"({time.time()-tf:.0f}s)")
        del model; gc.collect()   # free memory between folds — fixes time growth
...
def lgb_objective(trial):
    params = dict(
        objective         = "regression_l1",
        n_estimators      = N_EST_FINAL,   # same cap as full CV
        learning_rate     = 0.05,          # fixed — we tune structure, not lr
    ...
    oof_scores = []
    for fi, (tr, va) in enumerate(opt_folds):
        ...
        m.fit(
            ...
            callbacks=[
                lgb.early_stopping(stopping_rounds=40, verbose=False),
                lgb_verbose(period=200, label=f"[optuna t{trial.number} f{fi}] "),
            ],
        )
...
full_lgb = lgb.LGBMRegressor(**{**BEST_LGB_PARAMS, "n_estimators": min(N_EST_FINAL, 1500)})
p("    Training full LGB for importances (no CV) …")
full_lgb.fit(X_train, y_transformed,
             callbacks=[lgb_verbose(period=100, label="[imp] ")])
...
for i in tqdm(range(15), desc="    Null imp"):
    ...
    m.fit(X_train, ys, callbacks=[lgb_verbose(period=300, label=f"[null-imp {_}] ")])
